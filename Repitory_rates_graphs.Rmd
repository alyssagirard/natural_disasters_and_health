...
LA <- read_csv("~/Desktop/natural_disasters_and_health/data/rv-dashboard.csv")

LA <- LA %>% 
  mutate(LA = ifelse(is.na(COV_ADM), 0, COV_ADM)) %>% 
  mutate(LA = ifelse(is.na(FLU_ADM), 0, FLU_ADM)) %>% 
  mutate(LA = ifelse(is.na(RSV_ADM), 0, RSV_ADM))
